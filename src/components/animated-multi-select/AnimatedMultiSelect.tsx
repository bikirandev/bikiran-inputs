...
}> = ({ containerRef, options, handleSelect, position, name, formData }) => {
        ...
        {options.map((option: TSelectOption) => {
          const isActive: boolean =
            formData[name] !== undefined
              ? formData[name]?.indexOf(option.value?.toString()) !== -1
              : false;
          return (
            <OptionItem
              key={option.id}
...
const AnimatedMultiSelect: FC<TProps> = ({
  name,
  onChange,
  formData,
  label,
  ...
  const handleSelect = (val: any) => {
    ...
    const value =
      currentValue?.indexOf(val) !== -1
        ? currentValue?.filter((v: string) => v !== val) // Remove if already selected
        : [...(currentValue || []), val]; // Add if not selected
